...
Prisma structural parity validation **MUST** use narrow source scanning constrained to the current real fixture template style. It **MUST** parse model blocks, field `@map` column names, model `@@map` table names, mapped `@@index` declarations, and simple equality partial-index predicates. The template **MUST** remain valid for Prisma >=7.4 with `previewFeatures = ["partialIndexes"]`, no schema-file datasource `url`, explicit schema placeholders for real-target generation, datasource `schemas`, and per-model `@@schema` declarations. Prisma delivery `updatedAt` **MUST** use `@updatedAt`.
